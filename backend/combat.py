...
def update_action(state: list, current_action: dict):
    ...
    if current_action["Type"] == "Switch":
        ...
        time = state[player_turn]["Scraplings"][active_scraplings[player_turn]][
            "MoveBlock"
        ]["EffectiveValue"]

    elif current_action == "Chip":
        action_result = attack(
            state[player_turn]["Scraplings"][active_scraplings[player_turn]],
            state[1 - player_turn]["Scraplings"][active_scraplings[1 - player_turn]],
            speed["Normal"],
            accuracy["Normal"],
            damage["Normal"],
            armour_piercing["None"],
            1,
        )
        state[player_turn]["Scraplings"][active_scraplings[player_turn]][
            "DamageReduction "
        ...
        state[player_turn]["Scraplings"][active_scraplings[player_turn]]["Health "][
            "Value"
        ] += math.trunc(action_result[1] / 2)
        ...
        state[player_turn]["Scraplings"][active_scraplings[player_turn]]["Damage"][
            "NegativeMultiplier"
        ] = 1
